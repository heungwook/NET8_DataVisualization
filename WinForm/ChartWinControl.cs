...
namespace Orion.DataVisualization.Charting
{
    ...
    public class Chart : System.Windows.Forms.Control, ISupportInitialize, IDisposable
	{
		...
		/// </summary>
		public Chart(double renderingDpiX, double renderingDpiY, Size chartSizePx)
		{
			//*******************************************************
			...
			this.RenderingDpiX = renderingDpiX;
			this.RenderingDpiY = renderingDpiY;
			this.Size = chartSizePx;
		}
		...
		protected override void OnPaint(PaintEventArgs e)
		{
				...
				(this.BackColor == Color.Transparent || this.BackColor.A != 255) )
			{
				...
				try
				{
					this.chartPicture.ChartGraph.InitGraphics(null);
					RenderingGraphicsGDI renderGraphicsGdi = new RenderingGraphicsGDI(e.Graphics);
					if (this.paintTopLevelElementOnly)
					{
						chartPicture.Paint(renderGraphicsGdi, false);
					}
					chartPicture.Paint(renderGraphicsGdi, this.paintTopLevelElementOnly);
				}
				catch(Exception)
				{
			...
			else
			{
				...
				if(this.dirtyFlag || paintBufferBitmap == null)
				{
					...
					{
						this.chartPicture.ChartGraph.InitGraphics(null);
						RenderingGraphicsGDI renderGraphicsGdi = new RenderingGraphicsGDI(paintBufferBitmapGraphics);
						chartPicture.Paint(renderGraphicsGdi, this.paintTopLevelElementOnly);
					}
					catch(Exception)
					{
		...
		public void SavePDF(string lsPdfFile, System.Windows.Point chartLocation, PdfGraphicsInfo.EnumColorSpace lColorSpace, 
			OrionX2.OrionFont.FontManagerConfig fontMgr, bool lbUseKValueCMYKEqual)
		{
		...
		public void SavePDF(Stream lStrm, System.Windows.Point chartLocation, PdfGraphicsInfo.EnumColorSpace colorSpace, 
			OrionX2.OrionFont.FontManagerConfig fontMgr, bool lbUseKValueCMYKEqual)
        {
			try
			{
		...
		public void SavePDF(System.Windows.Point chartLocation, iTextSharp.text.Document pdfDoc, iTextSharp.text.pdf.PdfWriter pdfWrt, 
			PdfGraphicsInfo.EnumColorSpace colorSpace, OrionX2.OrionFont.FontManagerConfig fontMgr, bool lbUseKValueCMYKEqual)
		{
		...
		public void SavePS(
			System.Windows.Point chartLocation,
			PdfGraphicsInfo.EnumColorSpace colorSpace, OrionX2.OrionFont.FontManagerConfig fontMgr, 
			Dictionary<string, PSFontData> ldctPSFonts, Stream lStrmHeader, Stream lStrmBody)
		{
		...
		public Orion.DataVisualization.Charting.AntiAliasingStyles AntiAliasing
		{
			get
			{
