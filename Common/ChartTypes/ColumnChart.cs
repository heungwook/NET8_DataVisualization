...
#if Microsoft_CONTROL
	namespace Orion.DataVisualization.Charting.ChartTypes
#else
	namespace System.Web.UI.DataVisualization.Charting.ChartTypes
#endif
{
    ...
    internal class ColumnChart : PointChart
	{
			...
			Series seriesToDraw )
		{
			...
			foreach( Series ser in common.DataManager.Series )
			{
				...
				{
                    // Options
                    bool lbSkipLabel = (ser.cOptions.cbSkipEmptyLabels && string.IsNullOrWhiteSpace(point.AxisLabel));
                    if (lbSkipLabel)
                        continue;
		...
		protected virtual void DrawColumn2D( 
			...
			Series ser)
		{
            if (ser.cOptions.cbUseCustomBarColumnShape && !string.IsNullOrWhiteSpace(ser.cOptions.csSvgXml))
            {
                /*
                ...
                RectangleF rect = graph.GetAbsoluteRectangle(rectSize);
                ser.cOptions.cSvgXmlShape.Draw_Graphics(graph.Graphics, rect, point);
                */
                graph.FillRectangleCustomRel(rectSize, point, ser);
            }
            else
            {
