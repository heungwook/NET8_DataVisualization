...
#endif
{
    ...
    internal class PointChart : IChartType
	{
			...
			Series seriesToDraw )
		{
			...
			foreach( Series ser in common.DataManager.Series )
			{
				...
				foreach( DataPoint point in ser.Points )
				{
					...
					{
                        if (ser.cOptions.cbShowOverflowValues)
                        {
                            ProcessChartType_Label_Overflow(
                                selection,
                                graph,
                                common,
                                area,
                                ...
                                point,
                                xValue, yValue,
                                horizontalViewMax, horizontalViewMin,
                                verticalViewMax, verticalViewMin);
                        }
                        index++;
						continue;
					}
                    ...
                    // Options
                    if (ser.cOptions.cbLabelTopForMinusValue && yValue < 0)
                    {
                        markerPosition.Y = (float)VAxis.GetLinearPosition(0);
                    }
                    // Draw labels
                    DrawLabels( 
						area, 
        ...
        virtual protected void ProcessChartType_Label_Overflow(
            bool selection,
            ChartGraphics graph,
            CommonElements common,
            ChartArea area,
            ...
            double horizontalViewMax, double horizontalViewMin,
            double verticalViewMax, double verticalViewMin)
        {
            ...
            if (ser.cOptions.cbShowOverflowValues)
            {
                ...
                {
                    markerPosition.Y = (float)VAxis.GetLinearPosition(verticalViewMax);
                }
                else if (yValue < verticalViewMin)
                {
                    markerPosition.Y = (float)VAxis.GetLinearPosition(0);
                }
            }
