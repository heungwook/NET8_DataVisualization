		...
		internal class SmartLabel
		{
				...
				bool checkCalloutLineOverlapping)
			{
				...
				if(common.Chart.ShowDebugMarkings)
				{
					...
					if(positionFound)
					{
						graph.Graphics.DrawRectangle(new Pen(Color.Green), Rectangle.Round(graph.GetAbsoluteRectangle(lp)));
					}
					else
					{
				...
				bool checkCalloutLineOverlapping)
			{
					...
					labelPosition.Bottom > 100f || labelPosition.Right > 100f)
				{
					...
					{
						graph.Graphics.DrawRectangle(new Pen(Color.Cyan), Rectangle.Round(graph.GetAbsoluteRectangle(labelPosition)));
					}
#endif					
				...
				if(!collisionDetected && this.smartLabelsPositions != null)
				{
					...
					foreach(RectangleF pos in this.smartLabelsPositions)
					{
						...
						if(collision)
						{
							...
							{
								common.ChartPicture.ChartGraph.Graphics.DrawRectangle(new Pen(Color.Blue), Rectangle.Round(common.ChartPicture.ChartGraph.GetAbsoluteRectangle(pos)));
								common.ChartPicture.ChartGraph.Graphics.DrawRectangle(new Pen(Color.Red), Rectangle.Round(common.ChartPicture.ChartGraph.GetAbsoluteRectangle(labelPosition)));
							}
#endif
							collisionDetected = true;
							break;
				...
				bool checkCalloutLineOverlapping)
			{
				...
				foreach(RectangleF pos in this.smartLabelsPositions)
				{
					if(pos.IntersectsWith(labelPosition))
					{
						...
						if(graph != null && common.Chart.ShowDebugMarkings)
						{
							graph.Graphics.DrawRectangle(new Pen(Color.Blue), Rectangle.Round(graph.GetAbsoluteRectangle(pos)));
							graph.Graphics.DrawRectangle(new Pen(Color.Red), Rectangle.Round(graph.GetAbsoluteRectangle(labelPosition)));
						}
#endif
						collisionDetected = true;
						break;
