...
#endif
{
	...
	{
		// -ChartPDF
		public void GetPDF(PdfGraphicsInfo graphicsPdf)
		{
			RenderPDF renderingEnginePdf = new RenderPDF(graphicsPdf);
			ChartGraph.InitGraphics(renderingEnginePdf); 
					...
					try
					{
							...
							EmfType.EmfPlusOnly))
						{
							...
							using (Graphics metaGraphics = Graphics.FromImage(metaFile))
							{
								...
								RenderingGraphicsPDF renderGraphicsPdf = new RenderingGraphicsPDF(graphicsPdf, metaGraphics);
								this.PaintPdf(renderGraphicsPdf, false);
	...
	internal partial class ChartPicture
	{
		...
		internal void PaintPdf(
			IRenderingGraphics renderGraphicsPdf,
			bool paintTopLevelElementOnly)
		{
			...
			ChartGraph.Graphics = renderGraphicsPdf;
		...
		protected virtual void OnBeforePaintPdf(ChartPaintEventArgs e)
		{
			if (BeforePaint != null)
			{
		...
		/// </summary>
		/// <param name="e">Event arguments.</param>
		protected virtual void OnAfterPaintPdf(ChartPaintEventArgs e)
		{
			if (AfterPaint != null)
			{
				//Invokes the delegates.
