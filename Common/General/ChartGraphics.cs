    ...
    public partial class ChartGraphics : ChartElement
    {
            ...
            )
        {
           ...
           System.Drawing.Image image = _common.ImageLoader.LoadImage( name );
            ...
            {
                attrib.SetColorKey(backImageTransparentColor.ToColor(), backImageTransparentColor.ToColor(), ColorAdjustType.Default);
            }
                ...
                backColor != Color.Transparent)
            {
                ...
                using(Graphics graphics = System.Drawing.Graphics.FromImage(bitmap))
                {
                    ...
                    {
                        graphics.FillRectangle(backBrush.ToGdiBrush(), 0, 0, image.Width, image.Height);
            ...
            )
        {
            ...
            if(markerImage.Length > 0)
            {
                ...
                if (image != null)
                {
                    ...
                    {
                        attrib.SetColorKey(markerImageTransparentColor.ToColor(), markerImageTransparentColor.ToColor(), ColorAdjustType.Default);
                    }
                    ...
                    if (shadowSize != 0 && shadowColor != Color.Empty)
                    {
                        ...
                        attribShadow.SetColorKey(markerImageTransparentColor.ToColor(), markerImageTransparentColor.ToColor(), ColorAdjustType.Default);
            ...
            using (StringFormat drawingFormat = (StringFormat)format.Clone())
            {
                ...
                if (angle != 0 && angle != 90 && angle != -90)
                {
                    ...
                    if (boundaryRect != RectangleF.Empty)
                    {
                        ...
                        if (!region.IsEmpty(Graphics.GraphicsGdi))
                        {
                            this.Transform = oldTransform;
                            RectangleF truncateRect = region.GetBounds(Graphics.GraphicsGdi);
                ...
                if (labelImage != null)
                {
                    ...
                    {
                        attrib.SetColorKey(imageTransparentColor.ToColor(), imageTransparentColor.ToColor(), ColorAdjustType.Default);
                    }
        ...
        internal void FillRectangleCustomRel(RectangleF rectF,
            DataPoint point,
            Series ser)
        {
            //
            Color backColor = point.Color;
            ChartHatchStyle backHatchStyle = point.BackHatchStyle;
            string backImage = point.BackImage;
            ChartImageWrapMode backImageWrapMode = point.BackImageWrapMode;
            Color backImageTransparentColor = point.BackImageTransparentColor;
            ChartImageAlignmentStyle backImageAlign = point.BackImageAlignment;
            GradientStyle backGradientStyle = point.BackGradientStyle;
            ...
            Color shadowColor = ser.ShadowColor;
            int shadowOffset = ser.ShadowOffset;
            PenAlignment penAlignment = PenAlignment.Inset;
            bool circular = false;
            ...
            if (ser.cOptions.cSvgXmlShape == null || !ser.cOptions.cSvgXmlShape.csSvgxml.Equals(ser.cOptions.csSvgXml))
            {
                if (ser.cOptions.cSvgXmlShape != null)
                {
                    ser.cOptions.cSvgXmlShape.Dispose();
                    ser.cOptions.cSvgXmlShape = null;
                }
                ser.cOptions.cSvgXmlShape = new Options.SvgXmlInfo(ser.cOptions.csSvgXml);
            }
            ser.cOptions.cSvgXmlShape.Draw_Graphics(this.Graphics, rect, point);
...
System.Drawing.Image image = _common.ImageLoader.LoadImage( backImage );
        ...
        internal bool IsRightToLeft
        {
            get
            {
                ...
                return Common.ChartPicture.RightToLeft == System.Windows.Forms.RightToLeft.Yes;
            }
        }
