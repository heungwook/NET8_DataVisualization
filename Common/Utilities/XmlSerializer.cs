...
#endif
{
	...
	internal class BinaryFormatSerializer : SerializerBase
	{
        ...
        internal void WritePropertyValue(object obj, string elementName, BinaryWriter writer)
		{
			...
			{
				// Write as int
				writer.Write(((Color)obj).ToStringACMYKBytes());
			}
			...
			else if(obj is Color[])
			{
				...
				{
					writer.Write(color.ToStringACMYKBytes());
				}
			}
		...
		private bool SetPropertyValue(object obj, PropertyInfo pi, BinaryReader reader, bool skipElement)
		{
			if(pi != null)
			{
				...
				{
					// Read as string
					string lsColor = reader.ReadString();
					//objValue = Color.FromArgb(reader.ReadInt32());
					Color lValue;
					if (!Color.TryParseACMYKBytes(lsColor, out lValue))
						throw new Exception("Deserialize Color Error");
					objValue = (Color)lValue;
